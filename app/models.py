"""Shared data structures passed between the DWD sources, the FSI engine and the API."""
...
class WeatherPoint:
    ...
    All optional fields are ``None`` when DWD does not publish them, so the
    frontend can tell "no data" apart from "zero".
    """
...
@dataclass
class Warning:
    """A DWD warning, normalised to something the frontend can render directly."""

    event: str  # DWD's German event name, e.g. "Starke Hitze"
    event_en: str  # English label so international attendees can scan it
    headline: str
    description: str
    instruction: str
    severity: str  # minor | moderate | severe | extreme
    level: int  # DWD raw level
    kind: str  # thunderstorm | wind | rain | snow | ice | heat | ...
    region: str
    #: True for a "Vorabinformation": DWD signalling possible severe weather
    #: before it is certain enough to warn on. Rendered hatched, not solid.
    advance: bool = False
